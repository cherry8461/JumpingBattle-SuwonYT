...
function requestKisPayment(amount) {
    var reqType = "D1"; 
    var price = String(amount); 
    
    var intentUrl = "kisandagent://payment" + 
                    "?reqType=" + reqType + 
                    "&amount=" + price + 
                    "#Intent;" +
                    ...
                    "component=kr.co.kisvan.andagent/.InAppActivity;" + 
    ...
    try {
        ...
        triggerLink.href = intentUrl;
        document.body.appendChild(triggerLink);
        
        // 2번 알럿: 클릭 직전 가상 주소 확인
        alert("[추적 2] 가상 링크 생성 완료!\n주소: " + triggerLink.href);
        
        triggerLink.click();
        document.body.removeChild(triggerLink);
        
        // 3번 알럿: 클릭 명령이 브라우저에서 통과했는지 확인
        alert("[추적 3] 클릭 명령 전송 완료!\n이 창을 닫은 후 결제창이 안 뜨면 브라우저가 주소를 차단한 것입니다.");

    } catch (e) {
        // 4번 알럿: 자바스크립트 자체에서 에러가 터졌을 때 원인 포획
        alert("[에러 포획] 시스템이 거부함!\n원인: " + e.name + "\n메시지: " + e.message);
...
async function sendWalkInData() {
    // 🎯 주석을 해제하여 버튼 원복 안전장치와 정상 연동되도록 복구합니다.
    const btnAction = document.getElementById('btn-action');
    const txtAction = document.getElementById('submit-text');
    
    // 시작하자마자 버튼을 비활성화하여 중복 클릭 방지
    if (btnAction) btnAction.disabled = true;
    if (txtAction) txtAction.innerText = "신청 처리 중...";
    ...
    // -------------------------------------------------------------------------
    // [유효성 검사 구역] - 조건에 걸려 튕길 때 버튼을 다시 원복합니다.
    // -------------------------------------------------------------------------
    ...
    if (!teamValue) {
        ...
        if (txtAction) txtAction.innerText = "신 청 하 기";
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            // 단계 상태를 결제(PAY)로 완전히 변경
            currentStep = "PAY"; 
            console.log("[흐름체크] 서버 저장 성공! currentStep을 PAY로 변경 완료했습니다.");
            
            
            const finalBtn = document.getElementById('btn-action');
            const finalTxt = document.getElementById('submit-text');
            
            if (finalBtn) {
                finalBtn.disabled = false; // 🔓 잠금 강제 해제 (터치 가능 상태로 복구)
                finalBtn.removeAttribute('disabled'); // 구형 브라우저용 속성 강제 삭제
                finalBtn.style.pointerEvents = 'auto'; // 터치 이벤트 강제 활성화
            }
            if (finalTxt) {
                finalTxt.innerText = "결 제 하 기"; // 텍스트 변경
            }

            showCustomAlert("정보 등록이 완료되었습니다. [결제하기] 버튼을 한 번 더 눌러 카드를 넣어주세요.");
            return;
        }

        // ❌ 서버 에러 응답 처리
        let message = alertData.error; 
        try {
...
function onPaymentSuccess() {
    alert("카드 결제가 최종 완료되었습니다! 문이 열립니다.");
    location.reload(); // 성공 후 화면 초기화
}

function onPaymentFail(errorMsg) {
    alert("결제 실패: " + errorMsg + "\n다시 시도해주세요.");
}
/**
 * 버튼 클릭 및 엔터 입력 시 현재 단계(신청/결제)에 맞는 함수를 실행합니다.
 */
function handleActionProcess() {
    if (currentStep === "SUBMIT") {
        if (typeof sendWalkInData === "function") {
            sendWalkInData();
        }
        return;
    } 
    ...
    if (currentStep === "PAY") {
        try {
            // 🎯 화면의 입력창을 믿지 말고, 사장님 매장 단가(15,000원) 기준으로 
            // 현재 입력된 인원수를 다시 한번 강제로 쥐어짜냅니다.
            const adultInput = document.getElementById('adult_count');
            const childInput = document.getElementById('child_count');
            
            let adultCount = adultInput ? parseInt(adultInput.value, 10) || 0 : 0;
            let childCount = childInput ? parseInt(childInput.value, 10) || 0 : 0;
            
            // 만약 이미 앞 단계에서 리셋되어 0명이 나왔다면, 
            // 안전장치로 최소 1명 값(15,000원)을 주거나 아예 테스트용 100원을 강제로 밀어 넣습니다.
            let totalAmount = (adultCount + childCount) * 15000;

            if (totalAmount <= 0) {
                // 🌟 [테스트 치트키] 만약 화면 리셋 때문에 0원이 나왔다면, 
                // 테스트 결제용으로 1,000원(또는 100원)을 강제로 세팅해서 넘겨버립니다!
                console.log("[경고] 금액이 0원으로 인식되어 테스트 금액 15,000원으로 강제 격발합니다.");
                totalAmount = 15000; 
            }

            // 무적의 결제 함수 실행!
            requestKisPayment(totalAmount);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // [2] 팀명 관련 이벤트 (10자 제한 + 엔터 시 포커스 아웃)
    ...
    // [4] 전화번호 입력창 관련 이벤트 (숫자만 제한 + 엔터 시 단계별 액션 실행)
    ...
    if (phoneInput) {
        ...
        phoneInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                phoneInput.blur(); 
                handleActionProcess(); // 👈 상태에 따라 신청하기 또는 결제하기 실행
            }
        });
    }

    // [5] 언어 선택 드롭다운 토글 (모바일 대응)
