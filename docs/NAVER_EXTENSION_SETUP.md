...
4. 당일 취소는 `당일취소&노쇼`에 1건으로 자동 반영한 뒤 예약 데이터에서 삭제합니다.
5. 미래 날짜 취소는 카운트하지 않고 예약 데이터에서 삭제합니다.

성함·전화번호·팀명·난이도·인원은 수원영통 로컬 DB에만 저장합니다. 예약 링크·네이버 원문 응답은 저장하지 않습니다. 중복 취소 집계를 막기 위해 예약번호만 최소 이력으로 남깁니다.
