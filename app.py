...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS supply_history (
        ...
        cash_receipt_amount INTEGER DEFAULT 0,
        cash_expense_amount INTEGER DEFAULT 0,
    ...
    if 'cash_receipt' not in settlement_meta_columns:
        cur.execute("ALTER TABLE settlement_daily_meta ADD COLUMN cash_receipt INTEGER DEFAULT 0")
    ...
    if 'cash_receipt_amount' not in supply_columns:
        cur.execute("ALTER TABLE supply_history ADD COLUMN cash_receipt_amount INTEGER DEFAULT 0")
    if 'cash_expense_amount' not in supply_columns:
        cur.execute("ALTER TABLE supply_history ADD COLUMN cash_expense_amount INTEGER DEFAULT 0")
    cur.execute("""
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS naver_reservations (
                ...
                party_concept TEXT NOT NULL DEFAULT '',
                party_people TEXT NOT NULL DEFAULT '',
    ...
    for column_name in ("team_name", "difficulty", "phone", "party_concept", "party_people"):
...
def save_supply_history():
    ...
    for entry in data:
        cur.execute('''INSERT INTO supply_history (target_date, time, item, quantity, etc_text, card_amount, cash_amount, transfer_amount, cash_receipt_amount, cash_expense_amount, total_amount)
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
            target_date,
            entry.get('time'),
            entry.get('item'),
            ...
            int(entry.get('cashReceiptAmount', 0)),
            int(entry.get('cashExpenseAmount', 0)),
    ...
    try:
        ...
        cur.execute('''
            ...
            SET time=?, item=?, etc_text=?, quantity=?, 
                card_amount=?, cash_amount=?, transfer_amount=?, cash_receipt_amount=?, cash_expense_amount=?, total_amount=?
            WHERE id=?
        ''', (
            time,           # 1
            item,           # 2
            ...
            cash,           # 6
            transfer,       # 7
            int(data.get('cash_receipt_amount') or 0),
            int(data.get('cash_expense_amount') or 0),
            total,
            sid
        ))
...
def create_supply_item():
    ...
    try:
        cur.execute('''
            INSERT INTO supply_history (target_date, time, item, etc_text, quantity, card_amount, cash_amount, transfer_amount, cash_receipt_amount, cash_expense_amount, total_amount)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (target_date, '', '항목', '', None, 0, 0, 0, 0, 0, 0))
...
def get_settlement_overview():
    ...
    cur.execute(
        "SELECT cash_expense, cash_receipt, no_show_count FROM settlement_daily_meta WHERE target_date=?",
        (target_date,)
    )
    meta_row = cur.fetchone()
    cash_expense = int(meta_row['cash_expense'] or 0) if meta_row else 0
    cash_receipt = int(meta_row['cash_receipt'] or 0) if meta_row else 0
...
def get_settlement_cash_receipt():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT cash_receipt FROM settlement_daily_meta WHERE target_date=?",
        (target_date,)
    )
    row = cur.fetchone()
    conn.close()
    amount = int(row['cash_receipt'] or 0) if row else 0
    return jsonify({'target_date': target_date, 'cash_receipt': amount})


@web.route('/api/settlement/cash_receipt', methods=['PUT'])
def save_settlement_cash_receipt():
    target_date = _get_request_date(default_today=True)
    data = request.json or {}
    ...
    cur.execute(
        '''INSERT INTO settlement_daily_meta (target_date, cash_receipt, updated_at)
           ...
           ON CONFLICT(target_date) DO UPDATE SET
             ...
             updated_at=datetime('now', 'localtime')''',
        (target_date, cash_receipt)
    )
    conn.commit()
    conn.close()
    return jsonify({'status': 'success', 'target_date': target_date, 'cash_receipt': cash_receipt})
...
def get_today_init_bookings():
    ...
    query = """
        SELECT
            cache.booking_id,
            COALESCE(NULLIF(reservation.customer_name, ''), cache.masked_name),
            cache.use_time_key,
            cache.room_name,
            ...
            COALESCE(reservation.product_name, ''),
            COALESCE(reservation.party_concept, ''),
            COALESCE(reservation.party_people, '')
