...
import { STORE_NAME } from "../store-profile";
...
export default function ReserveForm({
  ...
  function advanceFromTeamInput(event: KeyboardEvent<HTMLInputElement>) {
  ...
  useEffect(() => {
    if (step !== 2 || !stepReady || !teamInputFocused) return;

    const keepTeamInputVisible = () => {
      const field = teamInputRef.current;
      if (!field) return;
      const viewport = window.visualViewport;
      const visibleTop = viewport?.offsetTop ?? 0;
      const visibleBottom = visibleTop + (viewport?.height ?? window.innerHeight);
      const safeBottom = Math.max(visibleTop + 120, visibleBottom - 160);
      const fieldBottom = field.getBoundingClientRect().bottom;
      if (fieldBottom <= safeBottom) return;
      window.scrollBy({
        top: Math.ceil(fieldBottom - safeBottom + 16),
        behavior: "smooth",
      });
    };

    const firstAdjustment = window.setTimeout(keepTeamInputVisible, 60);
    const keyboardAdjustment = window.setTimeout(keepTeamInputVisible, 320);
    return () => {
      window.clearTimeout(firstAdjustment);
      window.clearTimeout(keyboardAdjustment);
    };
  }, [step, stepReady, teamInputFocused]);
    ...
    <main className={`reserve-shell reserve-step-shell ${stepReady ? "has-step-nudge" : ""}`}>
      <header className="reserve-step-header">
        <div className="reserve-brand-row">
          ...
          <div>
            <p className="eyebrow">JUMPING BATTLE · {STORE_NAME}</p>
            ...
            <>
              ...
              <label className="reserve-field reserve-team-field">
                ...
                <input
                  ...
                  onFocus={(event) => {
                    setTeamInputFocused(true);
                    revealField(event);
                  }}
                  onBlur={() => setTeamInputFocused(false)}
                  onKeyDown={advanceFromTeamInput}
                  enterKeyHint="next"
                  placeholder="예) 번개슛"
                />
              </label>
      ...
      <footer className="reserve-footer">
        점핑배틀 {STORE_NAME} · 오늘 방문 고객 전용 예약
      </footer>
    </main>
  );
}
