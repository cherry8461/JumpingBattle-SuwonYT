...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.copyFromId) {
      ...
      const reservation = await copyReservationToNextSlot(copyFromId, operator.email);
      if (!reservation) {
    ...
    if (error instanceof Error && error.message === "CANCELLED_RESERVATION") {
      return Response.json({ error: "취소된 예약은 한판 더 복사할 수 없습니다." }, { status: 400 });
    }
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const reservation = await updateReservation(id, command, operator.email);
